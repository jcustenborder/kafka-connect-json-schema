...
public class SchemaResolverFactory {
  public static <R extends ConnectRecord<R>> SchemaResolver<R> getInstance(JsonConfig config) {
    if (JsonConfig.SchemaLocation.Url == config.schemaLocation) {
      return new UrlSchemaResolver(config.schemaUrl);
    } else if (JsonConfig.SchemaLocation.Inline == config.schemaLocation) {
